...
export function classifyRisk(
  ...
  if (value >= thresholds.danger) {
    return { level: 'danger', estimated, reason: `Lưu lượng ${value.toFixed(1)} vượt ngưỡng nguy hiểm.` };
  }
  if (value >= thresholds.warning) {
    return { level: 'warning', estimated, reason: `Lưu lượng ${value.toFixed(1)} vượt ngưỡng cảnh báo.` };
  }
  if (value >= thresholds.watch) {
    return { level: 'watch', estimated, reason: `Lưu lượng ${value.toFixed(1)} vượt ngưỡng theo dõi.` };
  }
...
export function percentileThresholds(values: number[]): Thresholds | undefined {
  ...
  const quantile = (percentile: number) =>
    clean[Math.min(clean.length - 1, Math.floor((clean.length - 1) * percentile))];
  ...
  return {
    watch,
    warning,
    danger: Math.max(warning, warning * 1.25),
    estimated: true,
  };
}
